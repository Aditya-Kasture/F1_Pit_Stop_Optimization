...
target_testing_race = "unitedstates2019"

test_dataset = excel_unboxer("./data2019/" + target_testing_race + ".xlsx")
...
for i in test_dataset.values.tolist():
    ...
    result_list.append([prediction, answer, i[0], int(i[0])-prediction, int(i[0])-answer])
    ct+=1
result_df = pd.DataFrame(result_list, columns=["prediction", "answer_final_pos_gained", "initial_position", "predicted_final_pos", "answer_final_pos"])
result_df.to_excel("./final_model_prediction_result/test2.2_2019without2019_dropped/" + target_testing_race + "_prediction_result_successful2.xlsx", index=False)